...
public class CircularAryLinkedList<E> {
    ...
    private static final int NULL = -1;    // 뒤쪽노드가 없음
    ...
    public void addFirst(E obj) {
        ...
        if (rec != NULL) {
        	...
        	if(head == tail) tail = rec;   			   /* 배열의 첫번쨰 노드일 경우 */    	
        	n[head].set(obj, ptr);
        }
    }
    ...
    public void removeFirst() {
        if (head != NULL) {                            // 리스트가 비어있지 않으면
            ...
            deleteIndex(head);            
            head = crnt = ptr;
			if (head == NULL) tail = NULL; /* 삭제 이전 노드가 1개였을 때 */
        }
    }
    ...
    public void removeLast() {
        if (head != NULL) {
            if (n[head].next == NULL)            // 노드가 하나만 있으면
                ...
                n[pre].next = head;                    // pre는 삭제 뒤의 꼬리 노드
                deleteIndex(ptr);
                tail = crnt = pre;
            }
        }
    ...
    public void remove(int p) {
        if (head != NULL) {
            if (p == head)                                // p가 머리 노드이면
                ...
                while (n[ptr].next != p) {//p의 직전 노드를 찾기 위함
                    ptr = n[ptr].next;
