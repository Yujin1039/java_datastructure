...
public class ArrayLinkedList<E> {
    ...
    public void purge(Comparator <? super E> c) {
    	...
    	while(ptr != NULL) {
    		crnt = n[ptr].next;
    		while(crnt != NULL) {
    			if(c.compare(n[ptr].data, n[crnt].data) == 0) {
    				removeCurrentNode();
    			}
    			crnt = n[crnt].next;
    		}
    		//삭제된 노드가 존재하는 경우, 비교대상인 ptr의 노드도 삭제
    		if(deleted != temp) {
    			int temp2 = n[ptr].next;
    			remove(ptr);
    			ptr = temp;
    		}else {
    			ptr = n[ptr].next;    			
    ...
    //Q5. 머리 노드로부터 n개 뒤 노드의 "데이터 참조"를 반환하는 메서드 (≠ 노드의 참조)
    public E retrieve(int n) {
    	int ptr = head;
    	
    	while(n > 0 && ptr != NULL) {
    		//그냥 n[ptr]로 작성하면 배열 n[]이 아닌 매개변수 n으로 인식하므로
    		// 배열을 의미하고 싶을 때는 "this." 필수
    		ptr = this.n[ptr].next;
    		n--;
    	}
    	//해당 위치에 데이터가 없는 경우의 처리 필수
    	return this.n[ptr].data != null ? this.n[ptr].data:null;
    }
}
