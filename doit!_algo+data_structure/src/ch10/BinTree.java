...
public class BinTree<K,V> {
	...
	public boolean remove(K key) {
		...
		while (true) {
			...
			if (cond == 0)                           // 같으면
				...
				parent = p;                          // 가지로 내려가기 전(=대체 노드를 찾기 위해)에 부모를 설정
		...
		if (p.left == null) {                         /* p에 왼쪽의 자식이 없음(자식이 0명, 우측 자식 1명) */
			if (p == root)                     
				root = p.right;
			else if (isLeftChild)
		...
		} else if (p.right == null) {                 /* p에 오른쪽 자식이 없음(좌측 자식 1명) */
			if (p == root)
				root = p.left;
			else if (isLeftChild)
		...
		} else {                                      /* p의 자식이 2명 이상일 때 */
			parent = p;                               /* 대체 노드의 대체 자식을 찾기 위해 대체노드를 부모로 설정 */
			...
			p.key  = left.key;                           //  left의 키값을 p(대체 노드)로 이동
			p.data = left.data;                          //  left의 데이터를 p(대체 노드)로 이동
			if (isLeftChild)                          /* p의 자손이 1명인 경우 */                          
				parent.left  = left.left;                // left를 삭제
			else                                      /* p의 자손이 2명 이상인 경우 */
	...
	private Node<K, V> getMinNode() {
		...
		while(p.left != null) {/* p가 아닌 p.left가 null이 되는지를 기준으로 삼아야 함 */
			p = p.left;
		}		
		return p;
	}
	...
	public K getMinKey() {
		...
		return minNode != null ? minNode.getKey():null;
	}
	...
	public K getMaxKey() {
		...
		return maxNode != null ? maxNode.getKey():null;
	}
	
	// 가장 큰 키 값을 갖는 노드의 데이터를 반환
	public V getDataWithMaxKey() {
		Node<K,V> maxNode = getMaxNode();
		return maxNode != null ? maxNode.getValue():null;
	}
}
